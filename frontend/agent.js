...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        ...
        newForm.addEventListener('submit', async (e) => {
            ...
            const username = newForm.querySelector('#username').value;
            ...
            try {
                ...
                if (response.ok) {
                    setCookie('agentToken', data.token, 1); // 1 day expiration
                    setCookie('agentId', data.agent.id, 1); // 1 day expiration
                    setCookie('agentName', data.agent.username, 1); // Store agent name
                    showDashboard();
                } else {
                    if (loginError) {
