...
const { sendEmail, sendAppointmentEmails } = require('./services/emailService');
...
app.post('/api/chat/request', async (req, res) => {
    try {
        ...
        try {
            ...
            try {
                await sendEmail('default', {
                    category: 'New Chat Request',
                    query: query,
                    userName: name,
                    userEmail: email,
                    mobile: mobile,
                    details: 'A new chat request has been submitted.'
                });
                console.log('Sent email notification for new chat request');
            } catch (emailError) {
...
io.on('connection', (socket) => {
    ...
    socket.on('agent_exit_chat', async ({ chatId, userSocketId, resolved, reason, userDetails }) => {
        let userMessage = '';
        if (resolved) {
        ...
        // Send email with chat details if user details are provided
        if (userDetails && (userDetails.email || userDetails.name)) {
            try {
                await sendEmail('default', {
                    category: 'Chat Session Details',
                    query: `Chat session details for ${userDetails.name || 'User'}`,
                    userName: userDetails.name || 'User',
                    userEmail: userDetails.email || 'No email provided',
                    details: userDetails,
                });
                console.log('Chat details email sent successfully');
            } catch (emailError) {
                console.error('Failed to send chat details email:', emailError);
            }
        }
...
app.post('/api/exec', async (req, res) => {
    try {
        ...
        try {
            ...
            // Return the response without sending an email
            // We'll send the email after the user submits the contact form
            return res.json({
                success: true,
                response: response.data.response || 'I\'m not sure how to respond to that.',
                escalated: needsEscalation,
                category: response.data.category // Include the category in the response
            });
 ...
 * @returns {boolean} True if the query needs human escalation
 */
 ...
 * @returns {Promise<boolean>} True if an agent is available
 */
